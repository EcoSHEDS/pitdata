...
function clickDot(){
 ...
 else if (state.onClick == "spp") {

   // Empty selectedID array
   state.selectedID = [];
   
   // get all data from the species of the selected individual
   state.speciesData = getDataSpecies(state.nodesRender,d.species);

   // Add ID's of the selected fish's species to selectedID
   state.selectedID = state.speciesData.map( function(d){ return(d.id) } );
...
function startTour () {
  ...
  intro.setOptions({
    ...
    steps: [
      ...
      {
        element: '#showNotEnc',
        intro: '<p>Use this button to show fish that were known to be alive, but were not captured (encountered).</p>'
      },
      {
        element: '#collapseTwo',
      ...
      {
        element: '#btn-help',
        intro: '<p>Click this button to repeat the tour.</p>'
      }
    ]
  })
  intro.start();
